...
export function Navigation({ route, onNavigate }: { route: Route; onNavigate: (r: Route) => void }) {
  ...
  return (
    <nav
      ...
      className="glass sticky bottom-0 z-30 mx-auto flex w-full max-w-md items-stretch justify-around rounded-t-2xl border-t border-border px-2 pt-1.5 safe-b"
    >
      {items.map((r) => {
        ...
        return (
          <button
            ...
            className={cx(
              'flex min-h-[3.25rem] flex-1 flex-col items-center justify-center gap-1 rounded-xl px-1 py-1 text-[11px] font-medium transition',
